...
-- Check the description of the crime happened at the given location and date.
SELECT description FROM crime_scene_reports WHERE month = 7 AND day = 28 AND year = 2021 AND street = "Humphrey Street";

-- Find the names of the witnesses. Crime report said that the witness mentioned "bakery".
SELECT name, transcript FROM interviews WHERE year = 2021 AND month = 7 AND day = 28 AND transcript LIKE '%bakery%' ORDER BY name;

-- Eugene's clue: The thief widhdrew some money. Find the people who withdrew money at the given ATM and date, and put thoses names in the 'Suspect List'.
SELECT name, atm_transactions.amount FROM people JOIN bank_accounts ON people.id = bank_accounts.person_id
  JOIN atm_transactions ON bank_accounts.account_number = atm_transactions.account_number WHERE atm_transactions.year = 2021
   AND atm_transactions.month = 7
   AND atm_transactions.day = 28
   AND atm_transactions.atm_location = 'Leggett Street'
   AND atm_transactions.transaction_type = 'withdraw';

-- Raymond's clue: As leaving the bakery, they called a person and talked for less than a minute. They asked the person on the other end of the call to buy a flight ticket of the earliest flight on July 29, 2021.
...
SELECT abbreviation, full_name, city
 ...
 -- The first flight is flying to New York City at 8:20 am. This could be the place where the thief went to.
...
SELECT name, bakery_security_logs.hour, bakery_security_logs.minute FROM people
 ...
 -- Bruce must be the thief since he is in all the four lists: List of passengers, list of people who did the specific atm transactions, list of people who called, and list of people who drove away in cars from the bakery.
